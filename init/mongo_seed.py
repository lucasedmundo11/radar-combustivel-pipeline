...
Coleções:
  ...
  - buscas_usuarios: buscas e filtros realizados pelos usuários.
...
Variáveis de ambiente:
  ...
  N           (default: 10000) — registros por coleção
"""
...
def doc_localizacao_posto(fake: Faker, posto_id: ObjectId) -> dict[str, Any]:
    geo = make_fake_geo(fake)
    return {
        "_id": ObjectId(),
        "posto_id": posto_id,
...
def _with_direct_connection(uri: str) -> str:
    parts = urlsplit(uri)
...
def _candidate_uris(base_uri: str) -> List[str]:
    candidates = [_with_direct_connection(base_uri)]
    if "mongo:27017" in base_uri:
        candidates.append(_with_direct_connection(base_uri.replace("mongo:27017", "localhost:27017")))
    candidates.append(LOCALHOST_DIRECT_URI)
    seen, unique = set(), []
    for u in candidates:
        if u not in seen:
            seen.add(u)
...
def get_client(base_uri: str) -> MongoClient:
    last_exc = None
    for uri in _candidate_uris(base_uri):
        client = MongoClient(uri, serverSelectionTimeoutMS=8000)
        try:
            client.admin.command("ping")
            print(f"[SEED] Conectado: {uri}")
            return client
        except PyMongoError as exc:
            client.close()
            last_exc = exc
    raise RuntimeError(f"Não foi possível conectar ao MongoDB.") from last_exc
...
def ensure_replicaset(client: MongoClient) -> None:
    try:
        ...
        try:
            ...
            time.sleep(3)
            print("[SEED] Replica set iniciado.")
        except OperationFailure:
            pass
...
def main() -> int:
    mongo_uri = os.environ.get("MONGO_URI", LOCALHOST_DIRECT_URI)
    db_name = os.environ.get("DB_NAME", "radar_combustivel")
    seed = int(os.environ.get("SEED", "42"))
    batch_size = int(os.environ.get("BATCH_SIZE", "2000"))
    n_target = int(os.environ.get("N", "10000"))

    random.seed(seed)
    Faker.seed(seed)
    fake = Faker("pt_BR")

    print(f"[SEED] Alvo: {n_target} docs/coleção | SEED={seed} | DB={db_name}")
    ...
    for name in ("postos", "eventos_preco", "buscas_usuarios", "avaliacoes_interacoes", "localizacoes_postos"):
        db[name].drop()

    print("[SEED] Gerando postos e localizações...")
    ...
    print("[SEED] Criando índices MongoDB...")
    ensure_indexes(db)

    total = sum(
        db[c].estimated_document_count()
        for c in ("postos", "eventos_preco", "buscas_usuarios", "avaliacoes_interacoes", "localizacoes_postos")
    )
    print(f"[SEED] Concluído. Total de documentos: {total}")
    client.close()
    return 0
